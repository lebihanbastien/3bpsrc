function [tmdn, ymdn, yma] = diff_corr_level_II(tmd, ymd, N2, dcs, inputType, isTimeFixed)
...
while(normC > 1e-6 && iter < 50)
    ...
    %----------------------------------------------------------------------
    for i=1:N2
        text(yma(i,1),yma(i,2), num2str(iter),'Color','r', 'FontSize',14)
    end
...
% Arrival
plot(yma(:,1),   yma(:,2), 'bo', 'LineWidth', 2);
quiver(yma(:,1),  yma(:,2), yma(:,4),  yma(:,5), 'Color', 'b');

% Departure
plot(ymdn(:,1),   ymdn(:,2), 'k+', 'LineWidth', 2);
quiver(ymdn(:,1),  ymdn(:,2), ymdn(:,4),  ymdn(:,5), 'Color', 'k');
