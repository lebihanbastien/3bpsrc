...
if(output.manifold.distanceToMoon > cr3bp.m2.Rm/cr3bp.L)
    ...
    while(iter < iterMax)
        ...
        if(params.computation.type == cst.computation.MATLAB)
            ...
            %-----------------------------
            if(user.isBCP)
                [~, yarc, tearc, yearc, ~] = ode45(@(t,y)bcfbp_derivatives_42(t,y,cr3bp.mu, user.initSunPos),tspan,output.manifold.ystatem, earth.options);
                
            else
                [~, yarc, tearc, yearc, ~] = ode45(@(t,y)cr3bp_derivatives_42(t,y,cr3bp.mu),tspan,output.manifold.ystatem, earth.options);
            end
        else
            %-----------------------------
        ...
        %Correction to be made on output.manifold.deltaV
...
% the following type : DV = deltaV * output.manifold.ystatem(4:6)
...
if(params.computation.type == cst.computation.MATLAB)
    ...
    %-----------------------------
    if(user.isBCP)
        [~, ~, ~, yearc, ~] = ode45(@(t,y)bcfbp_derivatives_6(t,y,cr3bp.mu, user.initSunPos), tspan, output.manifold.ystatem(1:6), earth.options);
        
    else
        [~, ~, ~, yearc, ~] = ode45(@(t,y)cr3bp_derivatives_6(t,y,cr3bp.mu), tspan, output.manifold.ystatem(1:6), earth.options);
    end
...
% hold on
% plot(yarc(:,1)*cr3bp.L*10^(-3) ,yarc(:,2)*cr3bp.L*10^(-3) , 'm', 'LineSmoothing','on');
% plot(yarc(end,1)*cr3bp.L*10^(-3) ,yarc(end,2)*cr3bp.L*10^(-3) , 'ko', 'LineSmoothing','on');
