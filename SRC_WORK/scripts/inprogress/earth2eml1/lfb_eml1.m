...
% Year: 2015
%-------------------------------------------------------------------------%
function [ output, isSolution ] = lfb_eml1(manifold_branch_stable, cr3bp,  earth, moon, user, params,  cst,  varargin)
%% Init
    ...
    if(output.flyby.distanceToMoon > cr3bp.m2.Rm/cr3bp.L)
        ...
        while(iter < iterMax)
            ...
            if(~isempty(yearc)) %if the event has occured
                ...
                if(norm(delta_correction) > 1e2*norm(output.flyby.ystatem(4:6)))
                    iter = iterMax+1;
                    isSolution = false;
                    warning('Correction is too big.');
                    norm(delta_correction)
                    norm(output.flyby.ystatem(4:6))
                end
        ...
        % LEO deltaV  in synodical frame (equal to deltaV_sid because the
        % DV are equal in both frames) because we can consider t = 0 in the 
        % change of coordinates at the time of the maneuver.
        ...
        output.Ttot = abs(manifold_branch_stable.termination_time(nsol) + tearc);
...
% the following type : DV = deltaV * output.flyby.ystatem(4:6)
...
% plot(yarc(end,1)*cr3bp.L ,yarc(end,2)*cr3bp.L , 'ko', 'LineSmoothing','on');
