        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    if(order>orderMax[model]) {
        ...
        "orderr is greater than the maximum order currently allowed: 30 for the CRTBP, 20 for the QBCP.");
    }
    ...
    if(fwrk > F_SEM){
        mexErrMsgIdAndTxt("custom:cm:fwrk",
                "fwrk should be equal to F_SEM (1) or F_EM (0).");
    }
    ...
    //---------------------------------------------------------------------
    // Fix some configuration parameters
    //---------------------------------------------------------------------
    int isNorm    = 1;
    int li_SEM    = 2;
    int pms       = PMS_GRAPH;
    ...
    //Init routine. The interval of projection is set to tproj = T/5
    init_orbit(orbit, &CM, &CMh, &Mcoc, &MIcoc, &Vcoc, &orbit_ofs, OFTS_ORDER, OFS_ORDER, 1, tspan[0], tspan[1], SEML.us.T/5.0, &driver, &SEML);
    ...
    //Plotting parameters
    //------------------------------------------
    //we plot every 2 days for the EM system and every 10 days for the SEM system.
    double tplot = 1.0//default value, never reached
    switch(fwrk)
    {
        case F_EM:
            tplot = 2.0*86400/SEML.cs.cr3bp.T;
            cout << "tplot = " << tplot << endl;
            break;
            
        case F_SEM:
            tplot = 10.0*86400/SEML.cs.cr3bp.T;
            break;
    }
    ...
    switch(outputType)
    {
        case NC: //Normalized-Centered Coordinates (x, p)
        ...
        case SYS: //System coordinates (X, P)
        ...
        case VSYS: //System coordinates (X, V)
