                 ...
                 double *mu)                 //cr3bp mass ratio
{
    ...
    switch(nvar)
    {
        ...
        case 48:
        {
            init_ode_structure(&ode_s,   T, T_root, eps_AbsTol, eps_RelTol, eps_Root, eps_Diff, 48, 1e-6, cr3bp_derivatives_48, NULL, mu);
            break;
        }
                        ...
                        double *mu)                 //cr3bp mass ratio
{
    ...
    *t = t0;
    ...
    int ki = 0;
    do{      
        gsl_odeiv2_evolve_apply (ode_s.e, ode_s.c, ode_s.s, &ode_s.sys, t, tf, &ode_s.h, y);   
        //Store the current state
        tv[ki] = *t;
        for(int i = 0; i < nvar; i++) yv[i][ki] = y[i];
        ki++;    
                 ...
                 Bcp3d *param)               //tdbcp parameters
{
    ...
    switch(nvar)
    {
        case 6:
        {
            init_ode_structure(&ode_s,   T, T_root, eps_AbsTol, eps_RelTol, eps_Root, eps_Diff, 6, 1e-6, tdbcp_derivatives_6, NULL, param);
            break;
        }
        case 42:
        {
            //init_ode_structure(&ode_s,   T, T_root, eps_AbsTol, eps_RelTol, eps_Root, eps_Diff, 42, 1e-6, tdbcp_derivatives_42, NULL, param);
            break;
        }
        default:   //if nvar !=6 && nvar != 42, return without integration
        {
            mexPrintf("ode78_tdbcp. Error: wrong number nvar of state variables. nvar must 6 or 42. return.");
            return;
        }
                        ...
                        Bcp3d *param)               //tdbcp parameters
{
    ...
    switch(nvar)
    {
        ...
        default:   //if nvar !=6 && nvar != 42, return without integration
        {
            mexPrintf("ode78_tdbcp_vec_var. Error: wrong number nvar of state variables. nvar must 6 or 42. return.");
            return 0;
        }
