function orbit = diff_corr_3D_bb_corrected(v0, cr3bp , orbit, params, cst)
% DIFF_CORR_3D_BB Differential correction to compute 3D periodic orbits of
% the CRTBP, symmetric with respect to the xz-plane (halo).
%
...
while(true)
    ...
    %   - ve is the final 42-dimensionnal state
...
%Orbit update
orbit.y0  = v0;   %initial state (corrected)
...
xlabel(strcat('X', ' (km)'));
ylabel(strcat('Y', ' (km)'));
title('Differential correction process (correction)');
if(iter==1)
